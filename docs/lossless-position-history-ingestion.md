...
The feature gate remains default-off for fresh environments; in production, automatic retention is enabled. No schema change and no migration were added; retention execution truth combines process-local scheduler telemetry with durable cursor alignment. Controlled rollout readiness assessment #3 returned GO FOR CONTROLLED ROLLOUT, and the controlled production rollout has since completed.
