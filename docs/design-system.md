...
The fleet dashboard is the first page-level consumer. Its order is `PageHeader`, operational scheduler status, a primitive-composed filter bar, tabular summary metrics, then the fleet data surface. Use `Card` only for bounded operational groups and `Badge` only with meaningful status text. The desktop table has an accessible caption and column headers; the narrow viewport swaps it for an explicitly owned, content-parity mobile card list at 768px. Retained last-good fleet data remains visible during refreshes, with `LoadingStatus` and contextual `Alert` feedback instead of a full-page replacement. This is a reusable pattern, not an invitation to duplicate dashboard-local control styling on future pages.
