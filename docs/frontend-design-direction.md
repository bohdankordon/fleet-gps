...
## Vehicle Detail family

Vehicle Detail uses one reusable open page header with vehicle identity, durable application-timezone metadata, permission-aware route Tabs for Overview, Trips, and Movement History, and page-specific actions. The shared shell and Overview are the human-accepted baseline for the vehicle family. Tabs perform real route navigation and preserve direct URLs and each page's independent server loader. Overview separates disabled state, connectivity, and GPS freshness; absence of a usable position or daily summary remains an explicit absence rather than fabricated zero data. Its three operational Cards and bounded recent-event List use native Ant Design components and application-owned responsive CSS. Operational detail and event surfaces use a top-to-bottom field reading order with restrained section grouping instead of nested Cards; horizontal matrices are reserved for genuinely comparative or tabular data. Primary operational metrics may receive modest icon and typography emphasis while remaining within Fleet and Map density. Recent-event location preview is deferred until after the broader frontend redesign.

Trips follows one analytical flow: Period Trigger → Unified Summary → Analysis Workspace. The compact trigger keeps the applied range and timezone visible while quick choices and strict custom temporal editing live in a disclosed period editor. The four related measurements share one summary surface, and chronology plus selected-item Map are two panes of one workspace rather than independent Cards. Trip, Stop, and GPS-gap records use restrained primary, success, and warning markers without coloring whole records or implying activity during unknown gaps. The desktop Map may remain sticky within the analysis workspace; at tablet and phone widths it moves before the chronology so a long record list never hides the primary selection result. Only observed trip paths and actual stop boundary fixes are shown—road routes, addresses, stop centers, and interpolation across gaps are never invented.
