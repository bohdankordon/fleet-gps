...
`mode1.dataPositions.distance` измеряется в метрах, `goTime` — в секундах, а строковый `maxSpeed` приходит в узлах и нормализуется в км/ч. `mode2.dataSpeed.speed` уже измеряется в км/ч. `routes-new` загружается только по запросу маршрута; единица nested position speed пока не подтверждена, поэтому она не публикуется. Opaque `attributes` и `network` не выходят из адаптера.
