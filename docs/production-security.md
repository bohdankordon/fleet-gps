...
After the pinned images are built or pulled, `npm run production:check -- --env-file .env.production` explicitly loads the exact production env file, validates Stage 22 deployment fields and Web inputs, validates the production Compose configuration, and runs the compiled API parser inside a temporary `--rm --no-deps` container from the pinned API runtime image. It requires no host `node_modules`, TypeScript, or other devDependencies and performs no database write or provider/Telegram request. The API Docker builder guarantees eQuGPS and API compilation with a clean lockfile install; the image-side preflight then proves the compiled parser and eQuGPS runtime artifact load successfully. Nest configuration is parsed again during module construction before listening. The production Next config phase and `instrumentation.register()` validate Web configuration as defense in depth. Missing security-critical values, malformed URLs/integers, malformed nonempty booleans, and known repository placeholder secrets fail with field names only. The BFF also enforces production cookie attributes. Use [`.env.production.example`](../.env.production.example) as the sanitized deployment-input template; never commit a real `.env`.
...
No Swagger/OpenAPI explorer, debug/test/fixture/acceptance production route, permissive CORS surface, or public production source-map download is present. Acceptance scripts remain repository-only. Dependency advisories are attempted read-only when registry access is available; dependency maintenance never runs automatic audit fixes or unrelated upgrades.
