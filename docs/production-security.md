...
The Stage 21 `npm audit --omit=dev --json` attempt could not reach the npm registry advisory endpoint in the restricted environment, so no advisory result is claimed and no remediation or upgrade was performed. Package-registry tooling traffic was attempted once; application/provider traffic remained zero.
