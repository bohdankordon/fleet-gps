...
История всех GPS-точек, токены, пароли и сырые ответы eQuGPS в БД не хранятся. Prisma пока не является Nest provider, поэтому startup и health-check не подключаются к PostgreSQL.
