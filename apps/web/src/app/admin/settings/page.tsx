...
export default async function AdminSettingsPage() {
  ...
  return <div><AdminSubnavigation /><header className="hero"><div><p className="eyebrow">{t("admin.settings.eyebrow")}</p><h1>{t("admin.settings.title")}</h1><p>{t("admin.settings.description")}</p></div></header>{settings ? <AdminSettingsForm initial={settings} /> : <p className="admin-error" role="alert">{t("admin.settings.loadError")}</p>}</div>;
}
