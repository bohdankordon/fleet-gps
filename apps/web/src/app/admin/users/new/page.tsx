...
export default async function NewAdminUserPage() { const [actor, { t }] = await Promise.all([requireAuthUser(), getServerI18n()]); if (actor.role !== "ADMIN") redirect("/forbidden"); return <div><AdminSubnavigation /><PageHeader eyebrow={t("admin.users.title")} title={t("admin.users.createTitle")} description={t("admin.users.createDescription")} /><AdminUserCreateForm /></div>; }
