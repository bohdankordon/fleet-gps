...
export async function PATCH(request: Request, { params }: Readonly<{ params: Promise<{ groupId: string }> }>): Promise<Response> { const { groupId } = await params; return forwardVehicleGroups(request, `/api/admin/vehicle-groups/${encodeURIComponent(groupId)}`, ["name", "color"]); }
