...
export default async function EventsPage({ searchParams }: Readonly<{ searchParams: Promise<Record<string, string | string[] | undefined>> }>) {
  const initialActionTime = new Date().toISOString();
  ...
  return <div><EventsClient initialData={events.status === "fulfilled" ? events.value : { items: [], nextCursor: null }} initialError={events.status !== "fulfilled"} initialSummary={summary.status === "fulfilled" ? summary.value : null} initialFilters={filters} initialActionTime={initialActionTime} /></div>;
}
