...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { const [resolution, { locale, preferenceMode, t }] = await Promise.all([resolveAuthUser(), getServerI18n()]); const user = resolution.kind === "authenticated" ? resolution.user : null; return <html lang={locale}><body><AntdRegistry><AntDesignProvider locale={locale}><I18nProvider locale={locale} preferenceMode={preferenceMode}><AuthProvider user={user}><AppShell skipLabel={t("navigation.skipToMain")} navigation={<AppNavigation />}>{children}</AppShell></AuthProvider></I18nProvider></AntDesignProvider></AntdRegistry></body></html>; }
