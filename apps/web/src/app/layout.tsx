...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { const [user, { locale, t }] = await Promise.all([getAuthUser(), getServerI18n()]); return <html lang={locale}><body><I18nProvider locale={locale}><AuthProvider user={user}><TooltipProvider><AppShell skipLabel={t("navigation.skipToMain")}>{children}</AppShell></TooltipProvider></AuthProvider></I18nProvider></body></html>; }
