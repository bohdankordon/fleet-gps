...
test("renders read-only policy, anchor, fleet, backfill, observation, and checkpoint status language", () => {
  ...
  for (const expected of ["История GPS", "История GPS: 90 дней", "План на контрольную точку", "2026-08-11T02:00:00.000Z", "Provider-disabled", "Целей обработано", "GPS-наблюдения", "Строк наблюдений", "COMPLETED", "RUNNING", "PENDING", "NONE", "Только чтение"]) assert.ok(html.includes(expected), expected);
