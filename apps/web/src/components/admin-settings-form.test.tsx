...
test("settings form renders only approved controls with explicit labels, units, guidance, and readonly geofence status", () => {
  for (const expected of ["admin.settings.timezone", "admin.settings.minimumDailyDistance", "admin.settings.positionFreshness", "admin.settings.citySpeedLimit", "admin.settings.outsideCitySpeedLimit", "admin.settings.speedTolerance", "admin.settings.speedConfirmations", "admin.settings.inactivityDistance", "admin.settings.inactivityDuration", "admin.settings.unitMeters", "admin.settings.unitKph", "admin.settings.unitSeconds", "admin.settings.unitMinutes", "admin.settings.generalGuidance", "admin.settings.speedGuidance", "admin.settings.inactivityGuidance", "cityGeofence.configured", "admin.settings.geofenceReadonly"]) assert.ok(source.includes(expected), expected);
  for (const hidden of ["telegramChatId", "dailyReportMinuteOfDay", "provider", "scheduler", "retention", "trip", "stop", "textarea"]) assert.equal(source.includes(hidden), false, hidden);
  ...
  assert.match(loading, /role="status"/); assert.match(loading, /common\.loading/);
});
...
test("save interaction guards duplicates, keeps edits on errors/conflicts, and updates revision only from a valid response", () => {
  ...
  assert.match(page, /admin\.settings\.loadError/);
});
