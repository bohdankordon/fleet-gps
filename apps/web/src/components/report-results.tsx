...
export function ReportResults({ data, rows, user, desktop, sort, direction, onSort, onSelect }: Context & Readonly<{
  ...
  const columns: ColumnsType<Row> = [
    { title: t("reports.table.vehicle"), ...sortable("name"), width: "24%", render: (_, row) => <div className="reports-identity"><ReportVehicleIdentity row={row} user={user} />{row.hasGpsData ? <Tooltip trigger={["hover", "focus"]} title={<>{t("reports.first")}: {formatReportTimestamp(row.firstObservationAt, locale, data.timezone)}<br />{t("reports.last")}: {formatReportTimestamp(row.lastObservationAt, locale, data.timezone)}</>}><button type="button" className="reports-evidence neutral-tooltip-trigger">{t("reports.observations", { count: formatNumber(locale, row.rawObservationCount) })}</button></Tooltip> : <span className="reports-muted">{t("reports.noGps")}</span>}</div> },
