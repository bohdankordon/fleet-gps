...
export function PositionHistoryPopulation({ anchor }: Props) {
  ...
  return <section className="admin-history-section admin-history-population" aria-labelledby="history-population-title">
    ...
    <AlertDialog open={confirming} onOpenChange={setConfirming} trigger={<Button disabled={pending}>{t("history.population.start")}</Button>} title={t("history.population.confirmTitle")} description={t("history.population.warning")} cancelLabel={t("common.cancel")} confirmLabel={pending ? t("history.population.running") : t("history.population.start")} loading={pending} onConfirm={() => void execute()}>
      <dl className="ui-dialog__metadata"><div><dt>{t("history.slices.to")}</dt><dd><time dateTime={anchor}>{displayedAnchor}</time></dd></div><div><dt>{t("history.population.maxWindows")}</dt><dd>{number(maxWindows)}</dd></div><div><dt>Provider-disabled</dt><dd>{t(excludeProviderDisabled ? "history.population.willSkip" : "history.population.willInclude")}</dd></div></dl>
    </AlertDialog>
