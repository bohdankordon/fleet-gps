...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button({ variant = "primary", size = "default", loading = false, iconBefore, iconAfter, fullWidth = false, className, children, disabled, type = "button", ...props }, ref) {
  const classes = ["ui-button", `ui-button--${variant}`, `ui-button--${size}`, fullWidth ? "ui-button--full-width" : "", className].filter(Boolean).join(" ");
  return <button {...props} ref={ref} type={type} className={classes} disabled={disabled || loading} aria-busy={loading || undefined}>
