...
export function DashboardClient({ initialData, initialQuery, initialSchedulerStatus }: Props) {
  ...
  return <div className="dashboard-page fleet-overview">
    <FleetHeader data={data} schedulerStatus={initialSchedulerStatus} refreshing={loading} onRefresh={retry} />
    <FleetToolbar query={query} sort={sort} hasFilters={hasFilters} onSet={set} onSort={setSort} />
...
function FleetHeader({ data, schedulerStatus, refreshing, onRefresh }: Readonly<{ data: DashboardVehiclesResponse; schedulerStatus: SchedulerStatusResponse | null; refreshing: boolean; onRefresh: () => void }>) {
  const { locale, t } = useI18n();
  return <PageHeader className="fleet-page-header" title={t("dashboard.title")} metadata={<><span>{t("dashboard.metadata.vehicles")} <strong>{data.summary.total}</strong></span><span>{t("dashboard.summary.online")} <strong>{data.summary.online}</strong></span><span>{t("dashboard.metadata.generated")} <time dateTime={data.generatedAt}>{formatGeneratedAt(data.generatedAt, data.timezone, locale)}</time></span><details className="fleet-service-status"><summary>{t("dashboard.serviceStatus")}</summary><SchedulerStatus initialStatus={schedulerStatus} timezone={dashboardTimezone(data)} /></details></>} actions={<Button size="sm" loading={refreshing} onClick={onRefresh} iconBefore={<RefreshCw />}>{refreshing ? t("common.refreshing") : t("common.refresh")}</Button>} />;
}

function FleetToolbar({ query, sort, hasFilters, onSet, onSort }: Readonly<{ query: DashboardQuery; sort: FleetSort; hasFilters: boolean; onSet: <K extends keyof DashboardQuery>(key: K, value: DashboardQuery[K]) => void; onSort: (value: FleetSort) => void }>) {
  const { t } = useI18n();
...
function ConnectionStatus({ vehicle }: Readonly<{ vehicle: Vehicle }>) { const { locale } = useI18n(); const config = vehicle.status === "online" ? { Icon: CheckCircle2, className: "fleet-status--online" } : vehicle.status === "offline" ? { Icon: WifiOff, className: "fleet-status--offline" } : { Icon: CircleHelp, className: "fleet-status--unknown" }; return <Badge variant="outline" className={`fleet-badge ${config.className}`}><config.Icon />{statusLabel(vehicle.status, locale)}</Badge>; }
