...
import { CarOutlined, ReloadOutlined, SearchOutlined } from "@ant-design/icons";
...
function fleetColumns(data: DashboardVehiclesResponse, locale: ReturnType<typeof useI18n>["locale"], t: ReturnType<typeof useI18n>["t"]): TableColumnsType<Vehicle> { return [
  { title: t("dashboard.table.vehicle"), key: "vehicle", className: "fleet-table__vehicle-column", width: "1%", minWidth: 240, onCell: centeredFleetCell, render: (_, vehicle) => <span className="fleet-table__vehicle-identity"><Text className="fleet-table__vehicle-icon" type="secondary" aria-hidden><CarOutlined /></Text><span className="fleet-table__vehicle-copy"><Link className="fleet-vehicle-link fleet-table__vehicle-link" href={`/vehicles/${vehicle.id}`}><Text className="fleet-table__vehicle-name" ellipsis={{ tooltip: vehicle.name }}>{vehicle.name}</Text></Link>{vehicle.disabled ? <Tag className="fleet-table__disabled-tag" color="default" variant="filled">{t("dashboard.vehicle.disabled")}</Tag> : null}</span></span> },
