...
export function DashboardClient({ initialData, initialQuery, initialSchedulerStatus }: Props) {
  ...
  return <div className="fleet-page">
    ...
    <Card size="small">
      <Flex className="fleet-toolbar" gap="small" wrap="wrap" align="center" aria-label={t("dashboard.filters.label")}>
        <Input.Search className="fleet-toolbar__search" aria-label={t("dashboard.filters.search")} allowClear value={query.search ?? ""} onChange={(event) => set("search", event.target.value || undefined)} placeholder={t("dashboard.filters.searchPlaceholder")} />
        <Select className="fleet-toolbar__select" aria-label={t("dashboard.filters.status")} popupMatchSelectWidth={200} value={query.status ?? ""} labelRender={({ label }) => <>{t("dashboard.filters.status")}: {label}</>} onChange={(value) => set("status", (value || undefined) as DashboardStatus | undefined)} options={[{ value: "", label: t("common.all") }, { value: "online", label: t("dashboard.status.online") }, { value: "offline", label: t("dashboard.status.offline") }, { value: "unknown", label: t("dashboard.status.unknown") }]} />
        <Select className="fleet-toolbar__select" aria-label={t("dashboard.filters.activity")} style={{ flexBasis: 230, minWidth: 210 }} popupMatchSelectWidth={230} value={query.activity ?? ""} labelRender={({ label }) => <>{t("dashboard.filters.activity")}: {label}</>} onChange={(value) => set("activity", (value || undefined) as DashboardActivity | undefined)} options={[{ value: "", label: t("common.all") }, { value: "below_threshold", label: t("dashboard.activity.belowThreshold") }, { value: "normal", label: t("dashboard.activity.normalDistance") }, { value: "no_data", label: t("dashboard.activity.noData") }]} />
        ...
        <Select className="fleet-toolbar__select" aria-label={t("dashboard.sort.label")} style={{ flexBasis: 220, minWidth: 205 }} popupMatchSelectWidth={220} value={sort} labelRender={({ label }) => <>{t("dashboard.sort.label")}: {label}</>} onChange={(value) => setSort(value as FleetSort)} options={[{ value: "name", label: t("dashboard.sort.name") }, { value: "freshness", label: t("dashboard.sort.freshness") }, { value: "speed", label: t("dashboard.sort.speed") }]} />
