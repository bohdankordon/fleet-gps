...
function FleetToolbar({ query, sort, loading, onSet, onSort, onRefresh }: Readonly<{ query: DashboardQuery; sort: FleetSort; loading: boolean; onSet: <K extends keyof DashboardQuery>(key: K, value: DashboardQuery[K]) => void; onSort: (value: FleetSort) => void; onRefresh: () => void }>) {
  ...
  const refresh = <Button type="primary" icon={<ReloadOutlined />} loading={loading} onClick={onRefresh}>{t("common.refresh")}</Button>;
  return <Card size="small"><Flex className="fleet-toolbar" vertical gap="middle" aria-label={t("dashboard.filters.label")}><div>{search}</div><Flex className="fleet-toolbar__zones" wrap="wrap" align="center"><Flex className="fleet-toolbar__zone" gap={12} wrap="wrap" align="center"><Text type="secondary">{t("dashboard.toolbar.filters")}:</Text><Flex gap="middle" wrap="wrap" align="center">{status}{activity}{disabled}</Flex></Flex><Flex className="fleet-toolbar__zone fleet-toolbar__zone--view" gap={12} wrap="wrap" align="center"><Text type="secondary">{t("dashboard.toolbar.view")}:</Text><Flex gap="middle" wrap="wrap" align="center">{ordering}{refresh}</Flex></Flex></Flex></Flex></Card>;
}
