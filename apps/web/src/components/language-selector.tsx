...
const LANGUAGE_CONTROL_WIDTH_LABELS = Object.freeze([
  ...SUPPORTED_LOCALES.map((value) => createTranslator(value)("language.automatic")),
  ...SUPPORTED_LOCALES.map((value) => NATIVE_LOCALE_NAMES[value]),
]);
...
export function LanguageSelector() {
  ...
  const selectedLabel = preferenceMode === "automatic" ? t("language.automatic") : NATIVE_LOCALE_NAMES[locale];
  return <div className="taxi-header__locale">
    <Dropdown open={open} onOpenChange={setOpen} menu={{ items, selectable: true, selectedKeys: [selectedKey], onClick: ({ key }) => { setOpen(false); void change(key as LocaleSelection); } }} trigger={["click"]} placement="bottomRight" disabled={pending}>
      <Button className="taxi-header__control taxi-header__locale-control" type="text" size="small" loading={pending} aria-label={t("language.label")} aria-expanded={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " " || event.key === "ArrowDown") { event.preventDefault(); setOpen(true); } }}>
        <span className="taxi-header__locale-stable">
          <span className="taxi-header__locale-sizing" aria-hidden="true">
            <GlobalOutlined aria-hidden />
            <span className="taxi-header__locale-sizing-labels">{LANGUAGE_CONTROL_WIDTH_LABELS.map((label) => <span key={label}>{label}</span>)}</span>
            <DownOutlined aria-hidden />
          </span>
          <span className="taxi-header__locale-content">
            <GlobalOutlined aria-hidden />
