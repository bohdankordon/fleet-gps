...
export function EventDetail({ event, onClose, now }: Readonly<{ event: AlertEvent; onClose: () => void; now: Date }>) {
  ...
  return <article className="event-detail" aria-label={t("events.detail")} style={{ fontFamily: token.fontFamily, fontSize: token.fontSize, color: token.colorText }}>
    ...
    {actions.length > 0 && <section><Typography.Title className="event-detail__section-title vehicle-overview__section-title" level={5} style={{ margin: 0, fontSize: token.fontSizeLG }}>{t("events.investigate")}</Typography.Title><div className="event-detail__actions">{actions.map((action) => <Button size="large" type={action.key === "eventTrip" ? "primary" : "default"} key={action.key} href={action.href} icon={action.key === "vehicle" ? <CarOutlined aria-hidden /> : action.key === "track" ? <HistoryOutlined aria-hidden /> : action.key === "trips" || action.key === "eventTrip" ? <NodeIndexOutlined aria-hidden /> : <EnvironmentOutlined aria-hidden />}>{t(`events.action.${action.key}`)}</Button>)}</div>
