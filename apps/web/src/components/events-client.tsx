...
export function EventsClient({ initialData, initialSummary, initialFilters }: Props) {
  ...
  return <>
    <header className="hero"><p className="eyebrow">{t("events.eyebrow")}</p><h1>{t("events.title")}</h1><p>{t("events.description")}</p></header>
    <section className="events-summary" aria-label={t("events.summary.label")}><article><span>{t("events.summary.total")}</span><strong>{summary?.open.total ?? "—"}</strong></article><article><span>{t("events.type.SPEEDING")}</span><strong>{summary?.open.speeding ?? "—"}</strong></article><article><span>{t("events.type.INACTIVITY")}</span><strong>{summary?.open.inactivity ?? "—"}</strong></article></section>
    <section className="filters events-filters" aria-label={t("events.filters.label")}><label>{t("events.filters.status")}<select value={list.filters.status ?? ""} onChange={(event) => change("status", (event.target.value || undefined) as AlertEventsStatus | undefined)}><option value="">{t("common.all")}</option><option value="OPEN">{t("events.filters.open")}</option><option value="RESOLVED">{t("events.filters.resolved")}</option></select></label><label>{t("events.filters.type")}<select value={list.filters.type ?? ""} onChange={(event) => change("type", (event.target.value || undefined) as AlertEventsType | undefined)}><option value="">{t("common.all")}</option><option value="SPEEDING">{t("events.type.SPEEDING")}</option><option value="INACTIVITY">{t("events.type.INACTIVITY")}</option></select></label><button type="button" onClick={() => void firstRequest(list.filters, "refresh")} disabled={list.loading}>{list.loading ? t("common.refreshing") : t("common.refresh")}</button></section>
    {(list.error || summaryError) && <section className="notice" role="alert"><WarningIcon className="notice-icon" /><div><strong>{list.error === "more" ? t("events.loadMoreError") : list.error === "first" ? t("events.loadError") : t("events.summaryError")}</strong><button type="button" onClick={retry}>{t("common.retry")}</button></div></section>}
    {(list.loading || list.moreLoading) && <p className="refresh" aria-live="polite">{list.moreLoading ? t("events.loadingMore") : t("events.loading")}</p>}
    {list.data.items.length === 0 ? <section className="empty"><h2>{empty.heading}</h2><p>{empty.text}</p></section> : <><div className="table-wrap events-table"><table><thead><tr><th>{t("events.table.vehicle")}</th><th>{t("events.table.type")}</th><th>{t("events.table.status")}</th><th>{t("events.table.opened")}</th><th>{t("events.table.notification")}</th><th>{t("events.table.details")}</th><th>{t("events.table.resolved")}</th></tr></thead><tbody>{list.data.items.map((event) => <tr key={event.id}><td><strong>{canOpenVehicles ? <Link href={`/vehicles/${event.vehicle.id}`}>{event.vehicle.name}</Link> : event.vehicle.name}</strong></td><td>{alertTypeLabel(event.type, locale)}</td><td><span className={alertTone(event.status)}>{alertStatusLabel(event.status, locale)}</span></td><td><time dateTime={event.openedAt}>{formatAlertTimestamp(event.openedAt, locale)}</time></td><td><span className={deliveryTone(event.notificationDeliveryStatus)}>{notificationDeliveryLabel(event.notificationDeliveryStatus, locale)}</span></td><td className="event-details">{alertDetailsLabel(event, locale)}</td><td>{event.resolvedAt === null ? "—" : <time dateTime={event.resolvedAt}>{formatAlertTimestamp(event.resolvedAt, locale)}</time>}</td></tr>)}</tbody></table></div><div className="mobile-list events-mobile-list">{list.data.items.map((event) => <article className="event-card" key={event.id}><div className="event-card-heading"><strong>{canOpenVehicles ? <Link href={`/vehicles/${event.vehicle.id}`}>{event.vehicle.name}</Link> : event.vehicle.name}</strong><span>{alertTypeLabel(event.type, locale)}</span></div><div className="badges"><span className={alertTone(event.status)}>{alertStatusLabel(event.status, locale)}</span><span className={deliveryTone(event.notificationDeliveryStatus)}>{notificationDeliveryLabel(event.notificationDeliveryStatus, locale)}</span></div><dl><div><dt>{t("events.table.opened")}</dt><dd><time dateTime={event.openedAt}>{formatAlertTimestamp(event.openedAt, locale)}</time></dd></div><div><dt>{t("events.table.resolved")}</dt><dd>{event.resolvedAt === null ? "—" : <time dateTime={event.resolvedAt}>{formatAlertTimestamp(event.resolvedAt, locale)}</time>}</dd></div><div className="event-card-details"><dt>{t("events.table.details")}</dt><dd>{alertDetailsLabel(event, locale)}</dd></div></dl></article>)}</div></>}
    {canLoadMoreAlertEvents(list) && <div className="load-more"><button type="button" onClick={() => void loadMore()} disabled={list.loading || list.moreLoading}>{list.moreLoading ? t("common.loading") : t("audit.loadMore")}</button></div>}
  </>;
}
