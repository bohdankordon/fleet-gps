...
export function EventsClient({ initialData, initialSummary, initialFilters }: Props) {
  ...
  return <>
    ...
    {(list.error || summaryError) && <section className="notice" role="alert"><span>⚠</span><div><strong>{list.error === "more" ? t("events.loadMoreError") : list.error === "first" ? t("events.loadError") : t("events.summaryError")}</strong><button type="button" onClick={retry}>{t("common.retry")}</button></div></section>}
