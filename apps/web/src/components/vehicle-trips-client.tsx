...
export function VehicleTripsClient({ vehicleId, initialData, initialRange, initialError }: Props) {
  ...
  return <>
    <header className="hero track-hero"><p className="eyebrow">{t("trips.eyebrow")}</p><h1>{t("trips.title")}</h1><p>{t("trips.description")}</p><div className="metadata"><Link href={`/vehicles/${vehicleId}`}>{t("track.backToVehicle")}</Link><Link href={`/vehicles/${vehicleId}/track`}>{t("vehicle.history")}</Link></div></header>
    ...
    {analysisError && <section className="notice" role="alert"><span>⚠</span><div><strong>{t("trips.loadError")}</strong><span>{t("trips.loadErrorText")}</span></div></section>}
    ...
    {noEvents && <p className="trip-neutral">{t("trips.noEvents")}</p>}
