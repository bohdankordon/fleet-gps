...
test("same-origin access, lifecycle, and force Telegram disconnect writes reach only their expected upstream route", async () => { const cases = [["PATCH", "/api/admin/users/id/access", ["role", "permissions"], { role: "USER", permissions: [] }], ["POST", "/api/admin/users/id/disable", [], {}], ["POST", "/api/admin/users/id/enable", [], {}], ["POST", "/api/admin/users/id/reset-password", [], {}], ["POST", "/api/admin/users/id/telegram/disconnect", [], {}]] as const; for (const [method, path, keys, body] of cases) { let called = ""; const response = await forwardAdminUsersToUpstream(request(path, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }), path, "http://api.test", keys, async (url) => { called = String(url); return Response.json({ ok: true }); }); assert.equal(response.status, 200); assert.equal(called, `http://api.test${path}`); } });
