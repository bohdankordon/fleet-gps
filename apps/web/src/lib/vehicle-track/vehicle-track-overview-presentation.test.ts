...
test("never joins two server segments and uses server gap summary rather than sampled timestamp gaps", () => {
  const response = overviewTrackFixture([
    overviewSegment([trackPoint("2026-08-01T00:00:00Z"), trackPoint("2026-08-01T00:20:00Z", { longitude: 28.5 })], 500),
    overviewSegment([trackPoint("2026-08-01T00:25:01Z", { longitude: 28.6 }), trackPoint("2026-08-01T00:45:01Z", { longitude: 28.7 })], 700),
  ], 11);
  const model = buildVehicleTrackOverviewPresentation(response);
  ...
  assert.equal(model.start?.point.observedAt, "2026-08-01T00:00:00Z"); assert.equal(model.end?.point.observedAt, "2026-08-01T00:45:01Z");
