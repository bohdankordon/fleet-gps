...
import { activeDurableRunResponseSchema, createDurableRunRequestSchema, durableRunBudgets, recentDurableRunsSchema, safeDurableRunSchema } from "./position-history-durable-run-contract";
...
test("safe run contract rejects lease/internal fields and recent is terminal-only and bounded", () => {
  ...
  assert.equal(activeDurableRunResponseSchema.safeParse({ active: run("PENDING") }).success, true); assert.equal(activeDurableRunResponseSchema.safeParse({ active: null }).success, true);
