...
test("disabled scheduler has an explicit normal label", () => { assert.equal(schedulerStateLabel({ ...validSchedulerStatus, enabled: false }, "ru"), "Отключено"); });
test("scheduler failure leaves dashboard render model available", () => { assert.equal(schedulerInitialErrorLabel(null, "ru"), "Не удалось загрузить состояние обновления."); assert.equal(schedulerInitialErrorLabel(validSchedulerStatus, "ru"), null); });
