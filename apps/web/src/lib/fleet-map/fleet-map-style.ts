...
export function fleetMapStyleUrl(value: string | undefined = process.env.NEXT_PUBLIC_MAP_STYLE_URL): string {
  ...
  try { const url = new URL(value.trim()); return url.protocol === "https:" && url.origin === "https://tiles.openfreemap.org" && !url.username && !url.password && !url.search && !url.hash ? url.toString() : DEFAULT_FLEET_MAP_STYLE_URL; }
