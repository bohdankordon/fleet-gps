...
test("populate UI source has exact defaults, confirmation gate, pending guard, safe result and no expansive controls", () => {
  ...
  for (const expected of ["history.population.title", "useState<Budget>(24)", "useState(true)", "AlertDialog", "history.population.confirmTitle", "history.population.checkpoint", "history.population.warning", "common.cancel", "history.population.start", "if (pendingRequest.current) return", "pendingRequest.current = true", "disabled={pending}", "history.population.already", "history.population.failed", "history.population.result", "history.population.processed", "history.population.rateLimits", "router.refresh()"] ) assert.ok(source.includes(expected), expected);
