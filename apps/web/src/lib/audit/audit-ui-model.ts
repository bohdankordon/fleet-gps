...
export function auditDetailPresentation(item: AuditReadItem, locale: AppLocale = DEFAULT_LOCALE): AuditDetailPresentation {
  ...
  switch (item.eventType) {
    ...
    case "RETENTION_EXECUTED": case "AUTOMATIC_RETENTION_EXECUTED": {
      const workState = "moreCheckpointWork" in item.details
        ? [t("audit.detail.moreCheckpointWork", { value: yes(item.details.moreCheckpointWork, locale) }), item.details.moreObservationWork === null ? t("audit.detail.observationWorkDeferred") : t("audit.detail.moreObservationWork", { value: yes(item.details.moreObservationWork, locale) })]
        : [t("audit.detail.remainingCheckpoints", { count: item.details.remainingFullyObsoleteCheckpoints }), t("audit.detail.remainingObservations", { count: item.details.remainingExecutableObservationCandidates })];
      return group("retention", t("audit.detail.retention"), [t("audit.detail.canonicalAnchor", { value: formatAuditTimestamp(item.details.canonicalAnchor, locale) }), t("audit.detail.cutoff", { value: formatAuditTimestamp(item.details.policyCutoff, locale) }), t("audit.detail.deletedCheckpoints", { count: item.details.deletedCheckpoints }), t("audit.detail.deletedObservations", { count: item.details.deletedObservations }), ...workState, t("audit.detail.stoppedByBudget", { value: yes(item.details.stoppedByBudget, locale) })]);
    }
