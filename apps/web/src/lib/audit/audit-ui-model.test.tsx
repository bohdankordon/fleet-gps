...
import { AUDIT_EVENT_LABELS, AUDIT_SETTINGS_FIELDS, auditDetailPresentation, auditDetailsLines, auditPermissionChanges } from "./audit-ui-model";
...
test("renders newest-first semantic chronology with all event labels and contextual inline detail", () => {
  ...
  const html = renderToStaticMarkup(<AuditEventTable items={items} selectedId={items[0]!.id} showInlineDetails />);
  ...
  for (const item of items) assert.ok(auditDetailsLines(item).length > 0, item.eventType);
});
...
test("derives permission additions and removals without repeating full sets", () => {
  ...
  const lines = auditDetailsLines(access);
...
test("settings presentation allowlists all 16 fields and never leaks unknown values", () => {
  ...
  for (const item of [unsafe, malformedKnown]) {
    const detail = auditDetailPresentation(item);
    assert.equal(detail.unavailable, true);
    const rendered = renderToStaticMarkup(<AuditEventTable items={[item]} selectedId={item.id} showInlineDetails />);
