...
test("events navigation uses /events filters and only user changes create history", () => {
  assert.equal(alertEventsHistoryPath({}), "/events?mode=active"); assert.equal(alertEventsHistoryPath({ status: "OPEN", type: "INACTIVITY" }), "/events?mode=active&type=INACTIVITY");
