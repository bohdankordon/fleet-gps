import { AlertEventsContractError, type AlertEventsVehicleOptions, type AlertEventsListResponse, type AlertEventsSummaryResponse } from "./alert-events-contract";
...
export function createAlertEventsVehicleOptionsRouteHandler(fetchOptions: () => Promise<AlertEventsVehicleOptions>) { return async (): Promise<Response> => { try { return Response.json(await fetchOptions()); } catch (error) { if (error instanceof AlertEventsContractError) return safeError(502); return safeError(503); } }; }
