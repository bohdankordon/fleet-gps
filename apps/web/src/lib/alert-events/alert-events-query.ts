...
export const ALERT_EVENTS_UNGROUPED_FILTER = "ungrouped";
export type AlertEventsFilters = Readonly<{ mode?: AlertEventsMode; status?: AlertEventsStatus; type?: AlertEventsType; vehicleId?: string; group?: string; period?: AlertEventsPeriod; from?: string; to?: string }>;
...
function parseGroup(value: string | null): string | undefined { if (value === null) return undefined; if (value === ALERT_EVENTS_UNGROUPED_FILTER) return value; if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) throw new AlertEventsQueryError(); return value; }
...
export function parseAlertEventsFilters(params: URLSearchParams, now = new Date()): AlertEventsFilters {
  ...
  const common = { mode, status: mode === "active" ? "OPEN" : "RESOLVED", type: parseType(params.get("type")), vehicleId: parseVehicle(params.get("vehicleId")), group: parseGroup(params.get("group")) } as const;
...
export function parseAlertEventsRequestQuery(params: URLSearchParams): AlertEventsRequestQuery {
  for (const key of ["status", "type", "vehicleId", "group", "from", "to", "limit", "cursor"]) if (params.getAll(key).length > 1) throw new AlertEventsQueryError();
  return { status: parseStatus(params.get("status")), type: parseType(params.get("type")), ...(params.has("vehicleId") ? { vehicleId: parseVehicle(params.get("vehicleId")) } : {}), ...(params.has("group") ? { group: parseGroup(params.get("group")) } : {}), ...parseAlertEventsRange(params.get("from"), params.get("to")), limit: parseLimit(params.get("limit")), cursor: parseCursor(params.get("cursor")) };
}
...
export function serializeAlertEventsRequestQuery(query: AlertEventsRequestQuery): string {
  ...
  for (const key of ["status", "type", "vehicleId", "group", "from", "to"] as const) if (query[key] && !(query.mode === "active" && (key === "from" || key === "to"))) params.set(key, query[key]);
