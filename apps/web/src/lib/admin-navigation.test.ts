...
test("ADMIN sees Audit while USER with every permission never does", () => {
  ...
  assert.equal(adminNavigationFor(admin).some((item) => item.href === "/admin/vehicle-groups"), true);
  assert.equal(adminNavigationFor(user).some((item) => item.href === "/admin/vehicle-groups"), false);
...
test("administration children remain permission-filtered and select only their own nested routes", () => {
  ...
  assert.deepEqual(adminItems.map((item) => item.href), ["/admin/users", "/admin/vehicle-groups", "/admin/settings", "/admin/audit", "/admin/history"]);
  assert.deepEqual(adminNavigationFor(admin, "en").map((item) => item.label), ["Users", "Vehicle groups", "Business settings", "Audit", "GPS history"]);
