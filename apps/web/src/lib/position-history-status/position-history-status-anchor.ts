...
export function positionHistoryAnchorHref(value: string): string | null {
  return parseVehicleTrackTimestamp(value) ? `/admin/history?${new URLSearchParams({ to: value })}` : null;
}
