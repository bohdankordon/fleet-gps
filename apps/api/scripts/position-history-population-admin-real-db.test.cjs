...
const admin = new PositionHistoryPopulationRunAdminService(database, new PositionHistoryPopulationRunCreationService(database));
...
test("Stage 18B USER create, safe active/recent reads, conflict, terminal release, and cleanup use PostgreSQL without provider work", async () => {
  ...
  try {
    ...
    userId = user.id;
    const first = await admin.create(user.id, { to: new Date("2026-08-11T02:00:00.000Z"), windowBudget: 500, excludeProviderDisabled: true }); runIds.add(first.id);
    ...
    await assert.rejects(admin.create(user.id, { to: new Date("2026-08-11T02:00:00.000Z"), windowBudget: 1000, excludeProviderDisabled: false }), PositionHistoryPopulationRunConflictError);
    ...
    const second = await admin.create(user.id, { to: new Date("2026-08-11T02:00:00.000Z"), windowBudget: 5000, excludeProviderDisabled: false }); runIds.add(second.id);
    ...
    const afterFailure = await admin.create(user.id, { to: new Date("2026-08-11T02:00:00.000Z"), windowBudget: 500, excludeProviderDisabled: true }); runIds.add(afterFailure.id);
