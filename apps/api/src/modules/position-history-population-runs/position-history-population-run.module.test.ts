...
test("module is callable-only, reuses Stage 14C and the Stage 17C lock, and registers no automatic execution", () => {
  ...
  assert.doesNotMatch(source, /onModuleInit|@Cron|ScheduleModule|setTimeout/);
  assert.equal(Reflect.getMetadata("controllers", PositionHistoryPopulationRunModule), undefined);
});
