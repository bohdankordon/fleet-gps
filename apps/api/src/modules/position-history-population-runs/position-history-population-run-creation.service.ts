...
import { AuditEventRepository, buildDurablePopulationCreatedAuditEvent, buildUserActor } from "../audit";
...
function valid(input: CreatePositionHistoryPopulationRunInput): boolean {
  ...
  if (input.initiatorType === PositionHistoryPopulationRunInitiatorType.USER) return input.requestedByUserId !== undefined && uuid.test(input.requestedByUserId) && typeof input.requestedByLoginSnapshot === "string" && input.requestedByLoginSnapshot.length > 0 && input.requestedByLoginSnapshot.length <= 64;
