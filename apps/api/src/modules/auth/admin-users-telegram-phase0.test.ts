...
function baseRow(status: "CONNECTED" | "BROKEN" | "DISCONNECTED" | null) { return { id: targetId, login: "target", normalizedLogin: "target", role: AuthRole.USER, vehicleAccessMode: "ALL", disabled: false, mustChangePassword: false, passwordHashVersion: 1, passwordSalt: new Uint8Array([1]), passwordHash: new Uint8Array([2]), passwordChangedAt: null, createdAt: now, updatedAt: now, permissions: [{ key: "reports.view" }], vehicleGroupGrants: [], vehicleGrants: [], telegramConnection: status === null ? null : { status } }; }
function fixture(status: "CONNECTED" | "BROKEN" | "DISCONNECTED" | null) {
  const stored = baseRow(status);
  const seenIncludes: unknown[] = [];
  const tx: any = { $executeRaw: async () => 1, authUser: { findUnique: async (args: any) => { seenIncludes.push(args.include); return { ...stored, permissions: [...stored.permissions] }; }, findUniqueOrThrow: async (args: any) => { seenIncludes.push(args.include); return { ...stored, permissions: [...stored.permissions] }; }, update: async ({ data }: any) => Object.assign(stored, data), count: async () => 2 }, authUserPermission: { deleteMany: async () => {}, createMany: async () => {} }, authUserVehicleGroupGrant: { deleteMany: async () => {}, createMany: async () => {} }, authUserVehicleGrant: { deleteMany: async () => {}, createMany: async () => {} }, vehicleGroup: { findMany: async () => [] }, vehicle: { findMany: async () => [] }, authSession: { deleteMany: async () => {} } };
...
test("Phase0 user 11: connected Telegram status survives access mutation response", async () => {
  ...
  const updated = await f.service.updateAccess(actor(), targetId, { role: "USER", permissions: ["reports.view"], vehicleAccess: { mode: "ALL", groupIds: [], vehicleIds: [] } });
...
test("Phase0 user 13: non-connected status remains truthful", async () => {
  for (const status of [null, "DISCONNECTED", "BROKEN"] as const) {
    ...
    assert.equal((await f.service.updateAccess(actor(), targetId, { role: "USER", permissions: ["reports.view"], vehicleAccess: { mode: "ALL", groupIds: [], vehicleIds: [] } })).telegramStatus, expected);
  }
});
...
test("Phase0 user 14: no secret relation fields exposed and authoritative include is status-only", async () => {
  ...
  assert.deepEqual(Object.keys(ADMIN_USER_INCLUDE as any).sort(), ["permissions", "telegramConnection", "vehicleGrants", "vehicleGroupGrants"]);
