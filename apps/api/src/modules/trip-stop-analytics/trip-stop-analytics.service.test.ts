...
import { DEFAULT_TRIP_STOP_ANALYTICS_POLICY } from "./trip-stop-analytics.constants"; import { UNRESTRICTED_VEHICLE_SCOPE } from "../vehicle-access/vehicle-access.service";

const testUserId = "00000000-0000-4000-8000-000000000001"; const unrestrictedScopes = { resolve: async () => UNRESTRICTED_VEHICLE_SCOPE } as unknown as import("../vehicle-access/vehicle-access.service").VehicleScopeService; const vehicle = Object.freeze({ id: "00000000-0000-4000-8000-000000000001", name: "Local vehicle" });
...
function service(snapshot: Awaited<ReturnType<TripStopAnalyticsRepository["getSnapshot"]>>): TripStopAnalyticsService {
  return new TripStopAnalyticsService({ getSnapshot: async () => snapshot }, { getSnapshot: async () => DEFAULT_TRIP_STOP_ANALYTICS_POLICY } as any, unrestrictedScopes);
}

test("known vehicle with zero observations returns a successful empty analysis", async () => {
  const result = await service({ vehicle, observations: [] }).analyze(vehicle.id, { from, to }, testUserId);
...
test("unknown vehicle produces the explicit analytics not-found error", async () => {
  await assert.rejects(service({ vehicle: null, observations: [] }).analyze(vehicle.id, { from, to }, testUserId), TripStopAnalyticsVehicleNotFoundError);
});

test("invalid and over-seven-day targets are rejected before repository access", async () => {
  let reads = 0;
  const analytics = new TripStopAnalyticsService({ getSnapshot: async () => { reads += 1; return { vehicle, observations: [] }; } }, { getSnapshot: async () => DEFAULT_TRIP_STOP_ANALYTICS_POLICY } as any, unrestrictedScopes);
  await assert.rejects(analytics.analyze(vehicle.id, { from, to: from }, testUserId), TripStopAnalyticsTargetError);
  await assert.rejects(analytics.analyze(vehicle.id, { from, to: new Date(to.getTime() + 1) }, testUserId), TripStopAnalyticsTargetError);
  assert.equal(reads, 0);
  await analytics.analyze(vehicle.id, { from, to }, testUserId);
...
test("service exposes derived summary and never asks the repository for provider data", async () => {
  ...
  const analytics = new TripStopAnalyticsService({ getSnapshot: async (vehicleId, range) => { received = { vehicleId, range }; return { vehicle, observations }; } }, { getSnapshot: async () => DEFAULT_TRIP_STOP_ANALYTICS_POLICY } as any, unrestrictedScopes);
  const result = await analytics.analyze(vehicle.id, { from, to }, testUserId);
