...
import { UNRESTRICTED_VEHICLE_SCOPE } from "../vehicle-access/vehicle-access.service"; import { PrismaTripStopAnalyticsRepository } from "./prisma-trip-stop-analytics.repository";
...
test("reads one vehicle and one deterministic inclusive observation projection without writes", async () => {
  ...
  const transaction = {
    vehicle: { findFirst: async () => ({ id: "00000000-0000-4000-8000-000000000001", name: "Vehicle" }), update: async () => { writes += 1; } },
  ...
  const result = await repository.getSnapshot("00000000-0000-4000-8000-000000000001", { from, to }, UNRESTRICTED_VEHICLE_SCOPE);
...
test("unknown vehicle returns immediately without an observation query", async () => {
  ...
  const transaction = { vehicle: { findFirst: async () => null }, vehiclePositionObservation: { findMany: async () => { observationReads += 1; return []; } } };
  const client = { $transaction: async (callback: (value: typeof transaction) => unknown) => callback(transaction) } as unknown as PrismaClient;
  const result = await new PrismaTripStopAnalyticsRepository({ getClient: () => client } as DatabaseService).getSnapshot("00000000-0000-4000-8000-000000000001", { from, to }, UNRESTRICTED_VEHICLE_SCOPE);
