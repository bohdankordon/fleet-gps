...
test("analytics module is database and vehicle access with one read controller and no provider/scheduler imports", () => {
  assert.deepEqual((Reflect.getMetadata("imports", TripStopAnalyticsModule) as Array<{ name: string }>).map((value) => value.name), ["DatabaseModule", "VehicleAccessModule"]);
