...
test("module compiles lazily, exports only lifecycle service, and performs no DB work", async () => {
  let calls = 0;
  ...
  try {
    ...
    assert.deepEqual(Reflect.getMetadata("exports", AlertEventsModule), [AlertEventsLifecycleService]);
