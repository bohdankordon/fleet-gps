...
test("speeding CONFIRMED newlyConfirmed maps to a typed OPEN command", () => {
  ...
  if (action.kind === "OPEN") assert.deepEqual(action.command, { type: "SPEEDING", vehicleId: VEHICLE_ID, observedAt: new Date(OBSERVED_AT), zone: "CITY", speedKph: 72, speedThresholdKph: 60 });
});
