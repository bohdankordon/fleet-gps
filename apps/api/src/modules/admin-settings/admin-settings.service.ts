...
import { buildSettingsUpdatedAuditEvent, type AuditUserActor, AuditEventRepository } from "../audit";
import { validateGeoJsonPolygon, validateInactivitySettings, validateMinimumDailyDistanceMeters, validatePositionFreshnessSeconds, validateRevision, validateRuleEnabled, validateSpeedSettings, validateTimezone, validateUpdatedAt } from "../alert-settings/alert-settings.validation";
import { SettingsChangeNotifier, type SettingsChangeKind } from "../alert-settings";
import { DatabaseService } from "../database/database.service";
import { validateTripStopAnalyticsPolicy } from "../trip-stop-analytics";
import { ADMIN_SETTINGS_FIELDS, type AdminSettingsPatch, type AdminSettingsResponse, AdminSettingsError, type StoredSettings } from "./admin-settings.types";
...
export class AdminSettingsService {
  public constructor(private readonly database: DatabaseService, private readonly audit: AuditEventRepository, private readonly notifier: SettingsChangeNotifier) {}
  public async get(): Promise<AdminSettingsResponse> { try { const row = await this.database.getClient().applicationSettings.findUnique({ where: { id: 1 }, select: SELECT }); if (!row) throw new AdminSettingsError("UNAVAILABLE"); return present(row); } catch (error) { if (error instanceof AdminSettingsError) throw error; throw new AdminSettingsError("UNAVAILABLE"); } }
  public async update(actor: AuditUserActor, input: unknown): Promise<AdminSettingsResponse> {
    const update = patch(input); let result: { row: StoredSettings; changes: ReturnType<typeof changes> };
    try { result = await this.database.getClient().$transaction(async (tx) => {
      const current = await tx.applicationSettings.findUnique({ where: { id: 1 }, select: SELECT }); if (!current) throw new AdminSettingsError("UNAVAILABLE");
      const next = validateMerged(current, update); const auditChanges = changes(current, next); if (!auditChanges.length) return { row: current, changes: auditChanges };
      const written = await tx.applicationSettings.updateMany({ where: { id: 1, revision: update.revision }, data: { timezone: next.timezone, minimumDailyDistanceMeters: next.minimumDailyDistanceMeters, positionFreshnessSeconds: next.positionFreshnessSeconds, speedRuleEnabled: next.speedRuleEnabled, citySpeedLimitKph: next.citySpeedLimitKph, outsideCitySpeedLimitKph: next.outsideCitySpeedLimitKph, speedToleranceKph: next.speedToleranceKph, speedingConfirmationUpdates: next.speedingConfirmationUpdates, inactivityRuleEnabled: next.inactivityRuleEnabled, inactivityDistanceMeters: next.inactivityDistanceMeters, inactivityDurationMinutes: next.inactivityDurationMinutes, tripMovementSpeedKph: next.tripMovementSpeedKph, tripMovementConfirmationSeconds: next.tripMovementConfirmationSeconds, tripStopConfirmationSeconds: next.tripStopConfirmationSeconds, tripDataGapSeconds: next.tripDataGapSeconds, cityGeofenceGeoJson: next.cityGeofenceGeoJson === null ? Prisma.DbNull : next.cityGeofenceGeoJson as Prisma.InputJsonValue, revision: { increment: 1 } } });
      if (written.count !== 1) throw new AdminSettingsError("CONFLICT"); const row = await tx.applicationSettings.findUniqueOrThrow({ where: { id: 1 }, select: SELECT }); await this.audit.append(tx, buildSettingsUpdatedAuditEvent(actor, { changes: auditChanges })); return { row, changes: auditChanges };
    }); } catch (error) { if (error instanceof AdminSettingsError) throw error; throw new AdminSettingsError("UNAVAILABLE"); }
    const reset = resetKinds(result.changes); if (reset.size) this.notifier.notify(reset); return present(result.row);
  }
}
