...
test("runtime status module is read-only database plus Stage 14B planner with one controller", () => {
  assert.deepEqual((Reflect.getMetadata("imports", PositionHistoryStatusModule) as Array<{ name: string }>).map((value) => value.name), ["DatabaseModule", "PositionHistoryHorizonModule", "PositionHistoryHorizonExecutionLockModule"]);
