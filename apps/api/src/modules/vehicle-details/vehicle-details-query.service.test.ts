...
function subject(value: StoredVehicleDetailsSnapshot, calls?: string[]): VehicleDetailsQueryService {
  ...
  return new VehicleDetailsQueryService(repository, { now: () => { calls?.push("clock"); return NOW; } });
}
...
test("unknown valid vehicle is a typed not-found after the repository snapshot", async () => {
  await assert.rejects(subject(snapshot({ vehicle: null })).getDetails(VEHICLE_ID), VehicleDetailsNotFoundError);
});
...
test("reuses fleet-map coordinate boundaries, invalid-position, speed, and freshness semantics", async () => {
  for (const [latitude, longitude] of [[-90, -180], [-90, 180], [90, -180], [90, 180]] as const) {
    ...
    assert.deepEqual((await subject(value).getDetails(VEHICLE_ID)).currentState?.position, { latitude, longitude, observedAt: CURRENT.fixTime.toISOString() });
  }
  for (const currentState of [{ ...CURRENT, latitude: 91 }, { ...CURRENT, longitude: Number.NaN }, { ...CURRENT, fixTime: new Date(Number.NaN) }]) {
    assert.equal((await subject(snapshot({ vehicle: { ...snapshot().vehicle!, currentState } })).getDetails(VEHICLE_ID)).currentState, null);
  }
  assert.equal((await subject(snapshot({ vehicle: { ...snapshot().vehicle!, currentState: { ...CURRENT, speedKph: null } } })).getDetails(VEHICLE_ID)).currentState?.speedKph, null);
  assert.equal((await subject(snapshot({ vehicle: { ...snapshot().vehicle!, currentState: { ...CURRENT, speedKph: -1 } } })).getDetails(VEHICLE_ID)).currentState?.speedKph, null);
  assert.equal((await subject(snapshot({ vehicle: { ...snapshot().vehicle!, currentState: { ...CURRENT, valid: false } } })).getDetails(VEHICLE_ID)).currentState?.speedKph, null);
});
...
test("freshness is inclusive at the boundary and stale beyond it or for provider state", async () => {
  ...
  const results = [];
  for (const currentState of values) results.push((await subject(snapshot({ vehicle: { ...snapshot().vehicle!, currentState } })).getDetails(VEHICLE_ID)).currentState?.freshness);
...
test("captures generatedAt exactly once after the complete repository read", async () => {
  ...
  const query = new VehicleDetailsQueryService(repository, { now: () => { calls.push("clock"); clockCalls += 1; return NOW; } });
  assert.equal((await query.getDetails(VEHICLE_ID)).generatedAt, NOW.toISOString());
...
test("maps both OPEN alert types deterministically and rejects duplicate or over-limit persisted OPEN state", async () => {
  ...
  await assert.rejects(subject(snapshot({ activeAlertsExceededLimit: true })).getDetails(VEHICLE_ID));
  await assert.rejects(subject(snapshot({ activeAlerts: [{ type: AlertEventType.SPEEDING, confirmedAt: NOW }, { type: AlertEventType.SPEEDING, confirmedAt: NOW }] })).getDetails(VEHICLE_ID));
});
...
test("recent events reuse type-specific and notification projections without repeated vehicle data", async () => {
  ...
  const response = await subject(snapshot({ recentEvents })).getDetails(VEHICLE_ID);
