...
const testAuth = { auth: { id: "00000000-0000-4000-8000-000000000099" } } as unknown as import("../auth/auth.types").AuthenticatedRequest; const ID = "00000000-0000-4000-8000-000000000001";
...
test("invalid vehicle UUID returns deterministic safe 400 without calling the query", async () => {
  ...
  await assert.rejects(controller.getDetails("not-a-uuid", testAuth), status(400, { statusCode: 400, error: "Bad Request" }));
...
test("valid vehicle UUID is normalized and returns details once", async () => {
  ...
  assert.deepEqual(await controller.getDetails(ID.toUpperCase(), testAuth), response);
...
test("unknown vehicle returns safe 404 and internal failures return safe 500", async () => {
  ...
  await assert.rejects(missing.getDetails(ID, testAuth), status(404, { statusCode: 404, error: "Not Found" }));
  ...
  await assert.rejects(failed.getDetails(ID, testAuth), status(500, { statusCode: 500, error: "Internal Server Error" }));
});
