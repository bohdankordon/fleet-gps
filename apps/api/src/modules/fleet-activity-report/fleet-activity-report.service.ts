...
import { TripStopAnalyticsPolicyService } from "../trip-stop-analytics";
...
export class FleetActivityReportService {
  ...
  public async getReport(range: TripStopAnalyticsRange): Promise<FleetActivityReport> {
    const [snapshot, policy] = await Promise.all([this.repository.getSnapshot(range), this.policy.getSnapshot()]);
    ...
    const vehicles: FleetActivityVehicleRow[] = snapshot.vehicles.map((vehicle) => {
      const analytics = analyzeTripStopObservations(grouped.get(vehicle.id) ?? [], range, policy);
