...
test("groups one bounded snapshot and aggregates the authoritative Stage 15A core for every persisted vehicle", async () => {
  ...
  const report = await new FleetActivityReportService(repository, { getSnapshot: async () => DEFAULT_TRIP_STOP_ANALYTICS_POLICY } as any).getReport({ from, to });
