...
import { FleetMapModule } from "./fleet-map.module";
...
test("FleetMapModule compiles lazily with DatabaseModule only and performs no SQL or network work", async () => {
  let databaseCalls = 0;
  const client = { $queryRaw: async () => { databaseCalls += 1; }, $disconnect: async () => {} } as unknown as PrismaClient;
  const module = await Test.createTestingModule({ imports: [FleetMapModule] }).overrideProvider(API_CONFIG).useValue(config).overrideProvider(DATABASE_CLIENT_FACTORY).useValue(() => client).compile();
  try {
    assert.ok(module.get(FleetMapController));
    assert.deepEqual(Reflect.getMetadata("imports", FleetMapModule), [DatabaseModule]);
    assert.equal(databaseCalls, 0);
  } finally { await module.close(); }
});

test("AppModule integrates FleetMapModule exactly once", () => {
  const imports = Reflect.getMetadata("imports", AppModule) as readonly unknown[];
  assert.equal(imports.filter((value) => value === FleetMapModule).length, 1);
});
