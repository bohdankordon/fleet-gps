...
test("FleetMapModule compiles lazily with database and vehicle access and performs no SQL or network work", async () => {
  let databaseCalls = 0;
  ...
  try {
    ...
    assert.deepEqual(Reflect.getMetadata("imports", FleetMapModule), [DatabaseModule, VehicleAccessModule]);
