...
import { PositionHistoryIngestionCursorModule } from "./position-history-ingestion-cursor.module";
...
test("cursor module is dormant and has no provider or scheduler dependency", async () => {
  let sql = 0;
  const client = { $queryRaw: async () => { sql += 1; }, $disconnect: async () => undefined } as unknown as PrismaClient;
  const module = await Test.createTestingModule({ imports: [PositionHistoryIngestionCursorModule] })
    .overrideProvider(API_CONFIG).useValue(config)
    .overrideProvider(DATABASE_CLIENT_FACTORY).useValue(() => client)
    .compile();
  try {
    assert.ok(module.get(PositionHistoryIngestionCursorService));
    assert.equal(sql, 0);
    assert.deepEqual((Reflect.getMetadata("imports", PositionHistoryIngestionCursorModule) as Array<{ name: string }>).map((value) => value.name), ["DatabaseModule"]);
    assert.deepEqual(Reflect.getMetadata("exports", PositionHistoryIngestionCursorModule), [PositionHistoryIngestionCursorService]);
