...
test("retention runtime composes the database and the existing shared mutation-lock module only", () => {
  assert.deepEqual((Reflect.getMetadata("imports", PositionHistoryRetentionModule) as Array<{ name: string }>).map((value) => value.name), ["DatabaseModule", "PositionHistoryHorizonExecutionLockModule"]);
