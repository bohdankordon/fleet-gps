...
const facts: PositionHistoryRetentionFacts = {
  observations: { total: 12, olderThanPolicyCutoff: 5, atOrAfterPolicyCutoff: 7, oldestObservedAt: new Date("2026-04-01T00:00:00Z"), newestObservedAt: new Date("2026-08-13T00:00:00Z"), vehiclesWithObservationsOlderThanCutoff: 3 },
  checkpoints: {
...
test("reuses the Stage 18C anchor and subtracts exactly 90 absolute UTC days", async () => {
  ...
  const service = new PositionHistoryRetentionService(repository, { now: () => new Date("1999-01-01T00:00:00Z") });
...
test("policy is independent of local timezone, DST, and the history page anchor", async () => {
  const repository = { inspect: async () => facts } as PositionHistoryRetentionRepository;
  const service = new PositionHistoryRetentionService(repository, { now: () => new Date("2026-08-13T10:00:00Z") });
...
test("empty facts retain nullable extrema and do not manufacture overlap safety", async () => {
  const empty: PositionHistoryRetentionFacts = {
    observations: { total: 0, olderThanPolicyCutoff: 0, atOrAfterPolicyCutoff: 0, oldestObservedAt: null, newestObservedAt: null, vehiclesWithObservationsOlderThanCutoff: 0 },
  ...
  const plan = await new PositionHistoryRetentionService({ inspect: async () => empty }, { now: () => new Date("2026-08-13T00:00:00Z") }).getRetentionPlan();
