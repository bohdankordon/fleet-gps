...
function service(value: StoredVehicleTrackOverviewSnapshot): VehicleTrackOverviewQueryService {
  ...
  return new VehicleTrackOverviewQueryService(repository, { now: () => now });
}

test("maps known empty and one-point history with explicit sampled overview semantics", async () => {
  const empty = await service(snapshot()).getOverview(vehicleId, from, to);
  ...
  const one = await service(snapshot({ rawPointCount: 1, segmentCount: 1, qualityWarningCount: 1, firstObservedAt: at, lastObservedAt: at, points: [onePoint] })).getOverview(vehicleId, from, to);
...
test("preserves all ordered distinct fixes below the target and leaks no persistence internals", async () => {
  ...
  const result = await service(snapshot({ rawPointCount: 3, segmentCount: 1, firstObservedAt: at, lastObservedAt: later, points })).getOverview(vehicleId, from, to);
...
test("keeps output bounded for a logical 50,000-row raw range and is deterministic", async () => {
  ...
  const firstResult = await query.getOverview(vehicleId, from, to);
  const secondResult = await query.getOverview(vehicleId, from, to);
...
test("captures generatedAt once after the complete repository snapshot", async () => {
  ...
  let clockCalls = 0;
  const query = new VehicleTrackOverviewQueryService(repository, { now: () => { calls.push("clock"); clockCalls += 1; return now; } });
  await query.getOverview(vehicleId, from, to);
...
test("uses raw segment membership so true gaps survive and sampling intervals do not create false gaps", async () => {
  ...
  const result = await service(snapshot({ rawPointCount: 18_000, segmentCount: 2, firstObservedAt: a, lastObservedAt: d, points })).getOverview(vehicleId, from, to);
...
test("distinguishes unknown and too-fragmented ranges and fails safely on corrupt repository state", async () => {
  await assert.rejects(service(snapshot({ vehicle: null })).getOverview(vehicleId, from, to), VehicleTrackOverviewNotFoundError);
  await assert.rejects(service(snapshot({ tooFragmented: true })).getOverview(vehicleId, from, to), VehicleTrackOverviewTooFragmentedError);
  await assert.rejects(service(snapshot({ rawPointCount: 1 })).getOverview(vehicleId, from, to), VehicleTrackOverviewStateError);
  const at = new Date("2026-08-02T00:00:00.000Z");
  await assert.rejects(service(snapshot({ rawPointCount: 1, segmentCount: 1, firstObservedAt: at, lastObservedAt: at, points: [point(at, { latitude: Number.NaN })] })).getOverview(vehicleId, from, to), VehicleTrackOverviewStateError);
  await assert.rejects(service(snapshot({ rawPointCount: MAX_OVERVIEW_POINTS + 1, segmentCount: 1, firstObservedAt: at, lastObservedAt: at, points: Array.from({ length: MAX_OVERVIEW_POINTS + 1 }, () => point(at, { segmentRawPointCount: MAX_OVERVIEW_POINTS + 1 })) })).getOverview(vehicleId, from, to), VehicleTrackOverviewStateError);
});
