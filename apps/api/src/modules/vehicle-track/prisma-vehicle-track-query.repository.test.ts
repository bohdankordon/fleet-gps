...
test("reads vehicle and inclusive ordered bounded history in one repeatable-read transaction", async () => {
  ...
  const transaction = {
    vehicle: { findUnique: async (args: unknown) => { calls.push(["vehicle", args]); return { id: vehicleId, name: "Taxi" }; } },
  ...
  assert.deepEqual(await repository.getSnapshot(vehicleId, from, to), { vehicle: { id: vehicleId, name: "Taxi" }, points: [point] });
  assert.deepEqual(calls, [
    ["vehicle", { where: { id: vehicleId }, select: { id: true, name: true } }],
...
test("unknown vehicle performs no history query", async () => {
  let pointQueries = 0;
  const transaction = { vehicle: { findUnique: async () => null }, vehiclePositionObservation: { findMany: async () => { pointQueries += 1; return []; } } };
  const client = { $transaction: async (callback: (value: typeof transaction) => unknown) => callback(transaction) };
  const repository = new PrismaVehicleTrackQueryRepository({ getClient: () => client } as unknown as DatabaseService);
  assert.deepEqual(await repository.getSnapshot(vehicleId, from, to), { vehicle: null, points: [] });
