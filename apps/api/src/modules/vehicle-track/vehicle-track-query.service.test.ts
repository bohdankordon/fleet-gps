...
import { VehicleTrackNotFoundError, VehicleTrackStateError, VehicleTrackTooDenseError } from "./vehicle-track.types";
...
function service(points: readonly StoredVehicleTrackPoint[], vehicle: { id: string; name: string } | null = { id: vehicleId, name: "Taxi" }) {
  ...
  return new VehicleTrackQueryService(repository, { now: () => now });
}

test("maps empty, single, and multiple point summaries with exact safe fields", async () => {
  const empty = await service([]).getTrack(vehicleId, from, to);
  assert.deepEqual(empty.summary, { pointCount: 0, firstObservedAt: null, lastObservedAt: null });
  const one = await service([point(from, { speedKph: 0, valid: false, outdated: true })]).getTrack(vehicleId, from, to);
  ...
  const multiple = await service([point(from), point(from, { latitude: 49.3, valid: true, outdated: false }), point(later)]).getTrack(vehicleId, from, to);
...
test("captures generatedAt once after the complete repository snapshot", async () => {
  ...
  let clockCalls = 0;
  const query = new VehicleTrackQueryService(repository, { now: () => { calls.push("clock"); clockCalls += 1; return now; } });
  await query.getTrack(vehicleId, from, to);
...
test("allows 10,000 points and rejects the 10,001st without truncation", async () => {
  ...
  assert.equal((await service(tenThousand).getTrack(vehicleId, from, to)).summary.pointCount, MAX_TRACK_POINTS);
  await assert.rejects(service([...tenThousand, point()]).getTrack(vehicleId, from, to), VehicleTrackTooDenseError);
});

test("distinguishes unknown vehicle and fails safely on corrupt or unordered stored data", async () => {
  await assert.rejects(service([], null).getTrack(vehicleId, from, to), VehicleTrackNotFoundError);
  await assert.rejects(service([point(from, { latitude: Number.NaN })]).getTrack(vehicleId, from, to), VehicleTrackStateError);
  await assert.rejects(service([point(to), point(from)]).getTrack(vehicleId, from, to), VehicleTrackStateError);
  await assert.rejects(new VehicleTrackQueryService({ getSnapshot: async () => ({ vehicle: { id: vehicleId, name: "Taxi" }, points: [] }) }, { now: () => new Date(Number.NaN) }).getTrack(vehicleId, from, to), VehicleTrackStateError);
});

test("preserves coordinate boundaries, nullable quality, and non-negative finite speed", async () => {
  const response = await service([point(from, { latitude: -90, longitude: -180 }), point(to, { latitude: 90, longitude: 180, speedKph: 12.5, valid: true, outdated: false })]).getTrack(vehicleId, from, to);
  assert.equal(response.points.length, 2);
  for (const invalid of [point(from, { longitude: 181 }), point(from, { speedKph: -1 }), point(from, { speedKph: Number.POSITIVE_INFINITY })]) await assert.rejects(service([invalid]).getTrack(vehicleId, from, to), VehicleTrackStateError);
});
